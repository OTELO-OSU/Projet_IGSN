...
- `packages/admin/src/samples/use-download-import-template.ts` plus a button on the sample list, mirroring `use-download-attachment.ts` and reusing `save-blob.ts`.
- `POST /admin/samples/import` accepts the filled workbook as multipart, following the attachment upload at `admin-routes.ts` (`c.req.valid("form")`, validated like `uploadSampleAttachmentSchema`): size cap, extension and media-type check, nothing else. Answers 202 and discards the file.
- `packages/admin/src/samples/import-sample-dialog.tsx`: file picker, submit, success/failure toast.

This phase exists to land the plumbing (route, auth, multipart limits, UI) with no parsing risk.

**Verification**: an api spec posting a fixture workbook gets 202 and a non-xlsx gets 415; an admin browser spec drives the dialog.
