...
# DNS A records for {igsn.,igsn-admin.,igsn-api.}$DOMAIN must point at the host EIP.
#
...
docker build --platform linux/amd64 -f packages/admin/Dockerfile \
	--build-arg VITE_API_URL="https://igsn-api.$DOMAIN" -t igsn-admin:preprod .
...
# 4. Ship the compose file and Caddyfile (not the env file: you copy that).
