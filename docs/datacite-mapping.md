...
| `production.collectionMethod`, `.collectionMethodDescription`                             | none                              | contract routes them to OMS `sam:samplingMethod`; `collectionMethodDescription` is also an open gap, see below                                                                                            |
...
### Open gaps to raise with the contract's authors

Four Core fields carry real data the contract routes nowhere in DataCite. Implemented as the contract says (dropped), these are questions for its authors, not decisions:

- `identification.localName`: should it ride as `titles[] { titleType: "AlternativeTitle" }`?
- `classification.natureOfSample` and `classification.sampleObjectTypes[0]`: the contract sends both to OMS `sf:specimenType` only. Should either also reach DataCite `subjects[]`?
- `production.samplingSite_name`: a second candidate for `geoLocationPlace` alongside `placeNames[0]` and `locationDescription`. Which should win?
- `production.collectionMethodDescription`: should it ride as `descriptions[] { descriptionType: "Methods" }`?
