...
- `Project.name`, `ProcessStep.description` (both the synthesis step and a sub-sample's own steps) and `currentRepository.organization` are optional in our pivot although Core requires them: none of the three is guaranteed by `samplePublishBlockers`.
