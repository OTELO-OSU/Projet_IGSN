...
**Enforcement is merge-not-reject.** `PUT /:id` overlays only editable paths onto the stored sample, so a payload touching a frozen field is silently dropped. That merge is the mass-assignment guard, and disabled form inputs are UX only. `PUT /service/samples/:igsn` (ADR 0036) reads the same lock maps through `mergePublishedEdit`, but rejects instead: `frozenFieldEdits` diffs the payload against the merge result and the route answers 403 `field_frozen` on any path sent with a different value, since a script has no disabled input to warn it; an omitted frozen field takes the stored value, so a script need not echo what it cannot change.
