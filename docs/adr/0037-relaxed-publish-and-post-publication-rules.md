...
**2026-09-18 narrowing.** The collector and operator names each split into a firstname/lastname pair (chief scientist, collector, collection curator, synthetic operator all did), so the lock holds seven entries, not five: `collectorFirstname` and `collectorLastname` replace `collectorName`, `operatorFirstname` and `operatorLastname` replace `operatorName`. Each person's publish blocker splits the same way, one code per empty half (`collector_firstname_missing`, `synthetic_operator_lastname_missing`...), so the admin error lands on the empty input rather than on its filled sibling.

**2026-09-18 correction: a published person freezes as one unit, not two names.** The line above ("ORCID stays out of the lock and editable") was superseded before shipping: a sample person may now link a registry account (`*UserId`) instead of typing a name, `domain/sample/contact-link.ts` holding that exclusivity. A person is one thing to freeze, not a firstname/lastname pair with a loose ORCID beside it, so each frozen person gained its `*UserId` and `*Orcid` entries alongside the name halves: `collectorUserId`, `collectorOrcid`, `operatorUserId`, `operatorOrcid` join the lock, nine entries in total. When the frozen `*UserId` is set, `mergePublishedEdit` keeps the link and nulls the three typed fields, since the stored row carries only one of the two forms; when it is null, the merge keeps the stored name halves and ORCID as before. The chief scientist and collection curator stay unfrozen and un-linked-lockable the same as before this ticket, since neither was in the lock to begin with.

**2026-09-24 narrowing.** The collection curator is dropped from the scientific context, so its fields leave this record.

**2026-09-25 narrowing.** `attachment_metadata_missing` now fires only on a missing resource type: an attached file's title and description leave the blocker, matching the relation title becoming optional the same day.

**Material hierarchy: `optional` becomes a frontier.** `TreeNode.frozenWhenPublished` already inherits: `frozenMaterialPrefix` walks to the first marked segment and opens everything below it. `optional` did not: it applied to the node alone, so a partial path just above a marked node was invalid while everything below it was valid, which cannot express "niveau 2 or deeper is always a valid stop". `optional` now inherits the same way, through `isOptionalAtOrAbove` (`domain/src/sample/path/is-optional.ts`), read by `material/is-complete.ts`, `type/is-complete.ts` and `design-system/src/lib/hierarchy.ts` (`canStopAtPath`). This is a cross-package contract: three other vocabularies already set `optional` (collection-method, resource-type, physiographic-environment).
...
Measured before shipping: replaying both readings (node-only vs. inherited) over every path of all four vocabularies changed zero verdicts for the three vocabularies that already used `optional` (collection-method 48 paths, resource-type 69, physiographic-environment 71, measured under its former name and shape) and changed exactly the 112 intended material verdicts out of 741 paths. A second reviewer reproduced this independently.
