...
## Amendment 2026-09-07: the public frontend signs in on the same client, through the admin callback

The public frontend (`packages/frontend`) now signs in through the same public PKCE client as admin, not a second client: same `client_id`, same scopes, same token policy. It first returned through its own `origin + /auth/callback`, which GaiaData's test client rejected (`Invalid redirect uri`, 2026-09-08): the client registers one exact URI, the admin callback, and a second one is a request the product owner refused. The frontend therefore signs in and out with `redirect_uri` and `post_logout_redirect_uri` set to `origin + /admin/auth/callback`, carrying its own path in the oidc `url_state` (sign-in) and `state` (sign-out). The admin `AuthGate` sends a `url_state` outside `/admin/` back to it with a full navigation before its identity and institution gates, so a reader without admin onboarding still lands on the sample page, and the admin provider's sign-out callback does the same with the sign-out state. Both apps share the tab's `sessionStorage` (the same `oidc.user` key) and the origin's `localStorage` state store, which is what makes one callback serve both; the frontend also sets the admin's per-tab signed-out flag (now `domain/auth/signed-out.ts`) so the admin shows its welcome screen instead of bouncing to the Keycloak login form. One exact URI per [REQ-PARAM-02](#gt-sso-requirements); see [gaiadata-client-provisioning.md](../gaiadata-client-provisioning.md). `safeReturnPath` and `signIn` moved to `packages/domain/src/auth/` so both apps share one implementation instead of two copies. No new ADR: same client, same model, no new decision.

## Amendment 2026-09-08: cross-app sign-out
...
**Same-origin broadcast.** `domain/src/auth/sign-out-broadcast.ts` writes a random value to the `igsn-sign-out` `localStorage` key on sign-out. Admin's `AuthGate` and the frontend's `AuthControls` both listen for the `storage` event and drop their own user (`auth.removeUser()`) on it. The admin tab shows the welcome screen instead of calling `signinRedirect`, deliberately: the signing-out tab may not yet have reached Keycloak's `end_session` endpoint, so a live SSO cookie would redirect the other tab straight back into a signed-in session.

**No server-side revocation.** OIDC Back-Channel Logout into the api was built and then dropped: it needs a second field change on GaiaData's client (the back-channel logout URL) and the product owner wants that request limited to the frontend redirect URI. The api keeps validating access tokens locally, so a tab that misses the broadcast keeps a working token for at most the 5 minute access token life, the same window as before this amendment. Revisit with GaiaData's deletion-signal answer ([REQ-USER-01](#gt-sso-requirements)), which would reuse the same transport.
