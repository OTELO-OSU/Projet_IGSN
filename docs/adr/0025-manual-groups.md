...
- **Publication freezes the field through the existing lock maps** in `published-field-lock.ts`, one entry (`manualGroupIds`) in the sample fields' map, no new mechanism. `mergePublishedEdit` projects the stored `manualGroups` objects to their ids, since the payload key carries ids.
