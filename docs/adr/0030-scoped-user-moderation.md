...
Accepted. Amended 2026-08-21, folded in below: a manual group manager curates its own groups, no detach may drop a member owning a published sample, and the `status` right also decides who the weekday digest reaches. Open PO question: whether a manager may moderate a super admin, hidden from every digest today.
...
**One match rule, per (caller, target)**: a target is in reach when its recorded `institutional_laboratory` is in the scope expanded to laboratory codes (`managedLaboratoryCodes`), or when it belongs to a managed manual group.
...
- The right derives from the group managed, never from the target's own organisme, nor from the path the manager reached it by.
...
- `status` and `institutions` are per (caller, target); the manual-group right is caller-based, since belonging to a managed group is what puts the target in reach; holding both kinds grants both rows.
...
**The weekday pending-accounts digest follows that same `status` right**, filtered in JS through `userManagementRights` rather than by the SQL clause, which over-includes by ORing in manual-group reach.
