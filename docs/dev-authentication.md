...
- The public frontend signs in with the same `igsn-admin` client through the admin callback `/admin/auth/callback`, which sends it back to the page it came from (`ADR 0006` amendment). A signed-in reader sees "Go to admin" / "Sign out" in the header and an "Edit" link on a published sample they can reach in admin (a `HEAD /admin/samples/:id` probe answers 200).
