...
export function LineageView({ lineage }: { lineage: SampleLineage }) {
  ...
  const centre = lineage.nodes.find((node) => node.generation === 0);
  const compact = trimLineage(lineage, centre?.id ?? "", COMPACT_LIMITS);
  ...
  return (
    <div className="relative mt-2 h-96 rounded border bg-sky-50/40">
        ...
        <Dialog>
          ...
          >
            <DialogHeader>
              ...
              {lineage.truncated ? <p>{m.lineage_truncated()}</p> : null}
            </DialogHeader>
