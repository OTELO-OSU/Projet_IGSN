...
const lineage = (
  ...
  truncated = false,
): SampleLineage => ({ nodes, edges, truncated });
...
describe("LineageView", () => {
  ...
  it("should say in the dialog that the lineage is too large to be shown whole", async () => {
    const screen = await render(lineage(family.nodes, family.edges, true));

    await screen.getByRole("button", { name: "Expand the lineage" }).click();

    await expect
      .element(
        screen.getByText("This lineage is too large to be shown in full here."),
      )
      .toBeInTheDocument();
  });
