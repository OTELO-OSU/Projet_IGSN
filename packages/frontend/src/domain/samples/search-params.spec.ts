...
describe("searchQueryParams", () => {
  ...
  it("should prefetch by search when text has a query", () => {
    ...
    ).toEqual({ page: 2, perPage: 50, search: "granite", filters: {} });
  });

  it("should prefetch by facet when text has no query", () => {
    expect(
      searchQueryParams({ engine: "text", nature: "rock_powder", page: 1 }),
    ).toEqual({
      page: 1,
      perPage: 50,
      search: undefined,
