...
describe("the public sample lineage", () => {
  ...
  pgTest(
    "should keep a node reached at two depths at its nearest generation",
    async ({ db }) => {
      // Arrange
      const ownerId = await owner(db);
  ...
  pgTest("should answer a cycle without hanging", async ({ db }) => {
    // Arrange
    const ownerId = await owner(db);
    const root = await insertLineageSample(db, ownerId, "Root");
    const child = await insertLineageSample(db, ownerId, "Child", [root.id]);
    await insertSampleParents(db, root.id, [child.id]);
    // Act
    const lineage = await getLineage(db, root.igsn!);
    // Assert
    expect(lineage.nodes.find((node) => node.id === root.id)?.generation).toBe(
      0,
    );
    expect(lineage.nodes).toHaveLength(2);
  });

  pgTest("should cap a chain deeper than the depth limit", async ({ db }) => {
    // Arrange
    const ownerId = await owner(db);
    const root = await insertLineageSample(db, ownerId, "Root");
    let previous = root;
    for (let depth = 1; depth <= 12; depth++) {
      previous = await insertLineageSample(
        db,
        ownerId,
        `Descendant ${String(depth).padStart(2, "0")}`,
        [previous.id],
      );
    }
    // Act
    const lineage = await getLineage(db, root.igsn!);
    // Assert
    expect(lineage.nodes).toHaveLength(11);
