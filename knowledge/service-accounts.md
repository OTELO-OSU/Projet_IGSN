...
- **`GET /service/samples`** lists the published samples in the account's own `managerScope`, forcing `status: "published"` server-side so a draft, withdrawn or tombstone sample never appears whatever the caller sends. It orders by IGSN ascending, a total order since a published sample's IGSN is unique and immutable, so offset paging stays stable while samples are edited. It takes `page` and `perPage` only; no filter and no sort are exposed yet, more sort options coming with the mapping. It returns the internal `{ data: Sample[], meta: { total } }` shape, archive contacts unredacted since the account already reaches inside its own moderation scope. This is not yet the IGSN Core pivot format; see `docs/igsn-core-mapping.md` for that mapping, not implemented.
