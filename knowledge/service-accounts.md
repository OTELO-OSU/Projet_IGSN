...
- **Request flow**: an accepted, signed-in frontend user opens "Ask for a service account" from the public header, names it and picks groups from their own trio and memberships; `POST /admin/currentUser/service-accounts/requests` mails every super admin a prefilled `/service-accounts/create?request=<json>` link, the requester becoming the owner.
