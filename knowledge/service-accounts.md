...
- **`GET /service/samples`** lists the published samples in the account's own `managerScope`, forcing `status: "published"` server-side so a draft, withdrawn or tombstone sample never appears whatever the caller sends. It takes `page` and `perPage` only, no other filter, no sort. It returns the internal `{ data: Sample[], meta: { total } }` shape, archive contacts unredacted since the account already reaches inside its own moderation scope. This is not yet the IGSN Core pivot format; see `docs/igsn-core-mapping.md` for that mapping, not implemented.
