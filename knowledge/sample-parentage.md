...
- **One `SampleView` renders both public statuses** (`sample-sections.tsx` / `withdrawn-sample-sections.tsx` build its sections), so the lineage section keeps its React instance when the reader opens a withdrawn sample from the graph, and the fullscreen dialog keeps its zoom and pan.
- **The public page shows the lineage as a graph, centred on the sample.** `GET /samples/:igsn/lineage` walks ancestors and descendants under one rule: a relative that left draft is named, so a withdrawn or tombstoned parent or child still appears, carrying ADR 0033's exception forward onto the graph. A withdrawn node links to its page, which resolves redacted and carries the graph itself; a tombstoned node carries `tombstone: true` and is named without a link, its page answering 404. A draft relative is absent and stops traversal past it. See ADR 0043.
