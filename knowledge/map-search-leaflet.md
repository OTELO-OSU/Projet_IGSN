...
- The box rides the `bbox` query param, lives in the URL, and filters with `ST_MakeEnvelope` against the generated `geom` column on the shared `location` table, through a correlated `exists` ([[sample-location]]). Antimeridian-crossing boxes are out of scope for v1, the domain schema enforcing `west <= east`.
