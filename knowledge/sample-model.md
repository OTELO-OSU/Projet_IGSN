...
relations:
  ...
  - type: depends_on
    target: sample-links-attachments
  - type: depends_on
    target: kysely-dbal
status: stable
---
...
- Sub-blocks: identity (name, specific name, nature, type), [[material-classification-ltree]], [[sample-location]] (`location: Location | null`), physical description, condition, scientific context, provenance, repository (archive plus two admin-only contacts).
- One-to-many children have their own tables: `sample_link` and `sample_attachment`, see [[sample-links-attachments]].
