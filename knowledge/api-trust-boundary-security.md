...
- **One admin read needs no role on the sample**: `GET /admin/samples/parents/:id`, for prefilling a sub-sample from its parent, answers a `published` sample whole to any authenticated caller (the archive contacts are hidden on the public site only, the admin shows them), one 404 for every other refusal. See [[sample-parentage]].
