...
- **`targetResourceType` is nullable in the schema but required to publish.** `relation_resource_type_missing` blocks publication when any relation has none; `attachment_metadata_missing` blocks it when any attachment has no resource type, or has neither a title nor a description. Both are publish gates only: `POST /:id/attachments` still accepts a bare upload with no metadata.
