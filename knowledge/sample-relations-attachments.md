...
- **A relation is DataCite-shaped**: `relationType` (13 values, `is_cited_by` to `other`), `identifierType` (13 values, `igsn`, `doi`, `url`, `ark`...), `identifier`, a required `targetTitle`, then nullable `targetResourceType` (the DataCite resourceTypeGeneral list), `description`, and the three scheme fields (`relatedMetadataScheme`, `schemeURI`, `schemeType`) that `hasMetadataScheme` allows only under `has_metadata`, refused otherwise by the schema and hidden by the form. `relationTypeInformation` was dropped, being write-only and never rendered; see ADR [0047](adr/0047-drop-relation-type-information.md).
- **`targetResourceType` is nullable in the schema but required to publish.** `relation_resource_type_missing` blocks publication when any relation has none; `attachment_metadata_missing` blocks it when any attachment lacks a resource type, a title or a description, all three being required. Both are publish gates only: `POST /:id/attachments` still accepts a bare upload with no metadata.
- **Only three identifier types are format-checked**, in `relation/model.ts`: `doi` (`https://doi.org/10.xxxx/...` or `doi:10.xxxx/...`), `url` (http or https) and `igsn` (`igsnSchema`, so a legacy CNRS/TOAE identifier passes, see [[igsn-identifier]]). The other ten are stored as typed. `IDENTIFIER_TYPES` declaration order is the picker order, the three validated ones first, and `identifierTypeLabel` keeps their canonical casing (`arXiv`, `RAiD`, `bibcode`) so they are not translated.
...
- In the admin, each relation and each attachment is one titled `fieldset` block (`N. <Format> Relation`, `Attachment N`) with its delete button in the header, and a blank relation row is never dropped silently: it fails the save on its required fields until filled or removed ([[form-kit-and-hidden-values]]).
- The "Resource format" dropdown adds a relation block; its identifier field leads the block, labelled with that relation's format (`identifierTypeLabel`, e.g. "IGSN", "DOI", "ARK") rather than a generic "Identifier". Field order is identifier, Title, Relation type, Resource type, the scheme fields (`has_metadata` only), Description.
