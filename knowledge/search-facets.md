...
- A query-schema drift guard and an API column allow-list keep the registry and the SQL in step; text facets keep a substring `ILIKE`, except the three person facets (`chiefScientist`, `collectorName`, `collectionCurator`), each stored as a firstname/lastname pair and matched token by token against both columns, in any order, accent-insensitive, with the same fuzzy fallback as the free-text search ([[sample-search]]).
