...
- Only `manualGroupIds`, `scientificContext.provenanceStatus`, `collectorFirstname`/`collectorLastname` (field sample), `collectionOrigin` (collection specimen) and `syntheticDetails.operatorFirstname`/`operatorLastname` carry an entry (ADR 0037), seven entries in all since each of the two person names counts as two. The matching ORCID (`collectorOrcid`, `operatorOrcid`) is not in the lock and stays editable, a deliberate choice. `name`, `nature`, `type`, `geologicalContextDescription`, `geomorphologicalEnvironment`, location, collection date and the other scientific-context and synthetic-details fields are editable.
