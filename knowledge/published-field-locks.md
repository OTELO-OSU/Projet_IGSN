...
- Only `manualGroupIds`, `scientificContext.provenanceStatus`, `collectorFirstname`/`collectorLastname` (field sample), `collectionOrigin` (collection specimen) and `syntheticDetails.operatorFirstname`/`operatorLastname` carry an entry (ADR 0037), seven entries in all since each of the two person names counts as two. No person carries a `*Orcid` field any more: it is read-only, resolved from the linked account, so there is nothing left to lock or leave editable there; see ADR 0049. `name`, `nature`, `type`, `geologicalContextDescription`, `physiographicEnvironment`, location, collection date and the other scientific-context and synthetic-details fields are editable.
- `repository.*` (archive and contacts) carries no entry, so all five fields stay editable after publication, as does `syntheticDetails` beyond `operatorFirstname`/`operatorLastname` ([[synthetic-details]]).
