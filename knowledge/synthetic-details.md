...
- Every field is nullish, so a half-filled draft saves; seven are required to publish, `operatorFirstname` and `operatorLastname` counting as two of them (`operatorName` split into that pair, indexed like the other person names, [[scientific-context]]). The operator, like every sample person, is EITHER a link to a registry account (`operatorUserId`) OR the typed name/ORCID trio, never both; see [[scientific-context]] and `domain/sample/contact-link.ts`. A link always satisfies the two name blockers below.
- The operator maps to Core's `extensions.experiment.operator`, its own slot, not a `responsibility` agent; see ADR [0047](../docs/adr/0047-synthesis-operator-core-slot.md).
- Publish blockers, all gated on a complete synthetic material: `synthetic_starting_material_missing`, `synthetic_starting_material_composition_missing`, `synthetic_final_product_missing`, `synthetic_synthesis_date_missing`, `synthetic_operator_firstname_missing` and `synthetic_operator_lastname_missing` (one per empty name half).
...
- Publication freezes every field but the last five: `temperature`, `pressure`, `experimentalProtocol`, `experimentPurpose` and `equipmentUsed` stay editable ([[published-field-locks]]). The operator freezes as one unit: `operatorUserId`, `operatorFirstname`, `operatorLastname` and `operatorOrcid` are all locked, and the merge keeps whichever form (link or typed) is stored, nulling the other.
- `/service` never carries `operatorUserId` in or out; see ADR [0048](../docs/adr/0048-service-api-not-authoritative-about-identity.md).
...
- `dateRangeSchema(codePrefix)` (`domain/sample/date-range.ts`) is the single factory behind both `synthesisDate` and `collectionDate`, each carrying the same `day | hour` precision and, at `hour`, a required IANA time zone; its order/future check issues `<prefix>_order` and `<prefix>_future` issue codes that `sample-draft-field-errors.ts` maps back to flat draft field names by regex over both prefixes.
