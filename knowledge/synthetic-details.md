...
- `dateRangeIssues(codePrefix)` (`domain/sample/date-range.ts`) is the shared order/future check behind `synthesisDate`'s `dateRangeSchema` and `collectionDate`'s `collectionDateSchema` ([[sample-model]]), issuing `<prefix>_order` and `<prefix>_future` issue codes that `sample-draft-field-errors.ts` maps back to flat draft field names by regex over both prefixes. `synthesisDate` stays day-only; `collectionDate` also carries an explicit `day | hour` precision.
