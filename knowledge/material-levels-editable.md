...
- The 13 niveau-1 nodes (the 6 children of `rock`, the 4 of `sediment`, the 3 of `extraterrestrial_rock`) each carry `{ optional: true, frozenWhenPublished: false }`. `mineral`, `fossil` and `synthetic_rock_mineral` carry neither: they have no sub-level, so they are already complete and wholly frozen.
