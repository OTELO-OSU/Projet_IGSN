...
`hierarchy-select-field.tsx` still exists, exporting the shared helpers (`Hierarchy`, `hierarchyChildren`, `canStopAtPath`, `hierarchyPathLabel`, `composeHierarchyValue`...) that `HierarchyField` imports, and `hierarchy-cascade.tsx` (`HierarchyCascade`) still drives the public frontend search facet, untouched by this change. A later change moves those pure helpers into a shared `lib/hierarchy.ts` and deletes the old per-level widget; until then the file stays, unused by the admin form.
