...
The pure helpers (`Hierarchy`, `hierarchyChildren`, `canStopAtPath`, `hierarchyPathLabel`, `hierarchyLevelItems`, `composeHierarchyValue`, `toHierarchyPath`) live in `packages/design-system/src/lib/hierarchy.ts`, imported by `HierarchyField`, by `hierarchy-cascade.tsx` (`HierarchyCascade`, which still drives the public frontend search facet), by the admin collection-method filter and by the admin draft schema. The per-level widget is deleted.
