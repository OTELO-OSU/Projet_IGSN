...
- One popover lists the current level's children when the search box is empty; a non-empty query instead lists every descendant below that level (`hierarchyDescendantItems`), each labelled as a breadcrumb from that level down (`"Sedimentary > Sand"`), and picking one fills every intermediate level at once. It stays open while drilling down and closes at a leaf, on Escape, or by picking "Stop here", the first item offered in append mode (never shown when re-editing an existing chip, since stopping there is already the current value).
