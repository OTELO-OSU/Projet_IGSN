...
- **A sample attaches to manual groups through `sample_manual_group(sample_id, group_id)`**, both columns FK-cascading; `sampleSchema` carries `manualGroups` (id plus name), `createSampleSchema` carries `manualGroupIds`. Nothing attaches a group automatically.
  - Only the sample's owner may set or clear them; a super admin reports as `"owner"` and so can fix a misattribution, choosing among the sample owner's memberships and managed groups.
  - The attachable set is the owner's memberships plus the groups they manage, plus whatever ids the sample already stores, so a group the owner has since left round-trips on save. One query, `attachableManualGroups` (`api/src/manual-group/attachable-manual-groups.ts`), resolves that set for sample create, sample update and the service-account request. Anything outside it is 422; a non-owner changing a sample's stored set is 403, resubmitting it unchanged is accepted.
