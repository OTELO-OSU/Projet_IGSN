...
- **Sub-sampling grants `contributor` automatically.** Creating a sample with a parent makes the parent's owner a contributor on the child, silently; the creator stays sole owner if they already own the parent. The owner is still mailed about the declaration itself, just not about the grant ([[mail-notifications]]). See [[sample-parentage]] and ADR 0024's amendment.
