...
- `optional: false`: this node MUST be refined; absent is not a valid stopping point on its own. `optional: true` is inherited: it marks a frontier, opening the node and everything under it as a valid stopping point, mirroring `frozenWhenPublished`. `isMaterialComplete` and `isSampleTypeComplete` read it through `isOptionalAtOrAbove` in place of a leaf rule.
