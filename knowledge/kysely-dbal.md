...
- Types Postgres has no Kysely equivalent for are declared as `string` and validated by Zod at the boundary: `material` (ltree), `geom` on `location` (`Generated<string>`, never selected).
