...
- A whole family of blockers may be gated on the material, the six `synthetic_*` codes applying to a synthetic sample alone: `synthetic_starting_material_missing`, `synthetic_starting_material_composition_missing`, `synthetic_final_product_missing`, `synthetic_synthesis_date_missing`, `synthetic_operator_firstname_missing` and `synthetic_operator_lastname_missing` ([[synthetic-details]]).
- Each of the four sample person names is a firstname/lastname pair carrying one blocker per half (`<person>_firstname_missing` / `<person>_lastname_missing`), so the error lands on the empty input ([[scientific-context]]).
- An optional person half-filled blocks publish even though the whole person is optional, the filled half making the other one required ([[scientific-context]]).
