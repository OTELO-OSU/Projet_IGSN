...
- A signed-in frontend user requests one from the public footer; the owner alone rotates its API key (SHA-256 hash at rest, shown once) from admin Settings, and `/service` is the resulting machine mount, `GET /service/ping` its first route, 403 for a missing or unknown key; see ADR 0036.
