...
- An accepted user requests one from the Services section of admin Settings, where the owner alone rotates its API key (SHA-256 hash at rest, shown once), and `/service` is the resulting machine mount, `GET /service/samples` listing every published sample and narrowing to the account's own `managerScope` on `?editable=true`, `POST /service/samples` creating and publishing one owned by the account's owner and snapshotting the account's own trio, 403 for a missing or unknown key; see ADR 0036.
